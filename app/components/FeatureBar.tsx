...
export default function FeatureBar() {
  ...
  return (
    <section className="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-2">
      <div className="rounded-2xl bg-white border border-pink-100 p-3 sm:p-4 shadow-xs">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {features.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-center gap-2.5 p-1.5 rounded-xl hover:bg-pink-50/40 transition-colors"
              >
                <div
                  className={`w-8 h-8 shrink-0 rounded-xl ${item.bgColor} flex items-center justify-center border border-pink-100`}
                >
